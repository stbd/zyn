...
def _get_client_cookie(handler):
    cookie_user_id = None
    try:
        cookie_user_id = int(handler.set_signed_cookie(
            COOKIE_NAME,
...
#
    def get(self, args, kwargs):
        ...
        cookie = _get_client_cookie(self)
        if cookie is None:
...
class LoginHandler(tornado.web.RequestHandler):
    ...
    def post(self, args, kwargs=None):
        ...
        if session.authenticate():
            log.debug(f'Login successfull for {username}')
            user_id = user_sessions.add(session)
            self.set_secure_cookie(COOKIE_NAME, str(user_id), expires_days=COOKIE_DURATION_DAYS)
            self.redirect('/fs/', permanent=True)
        else:
            log.debug(f'Login unsuccessfull for {username}')
